...
class App extends Component {
  ...
  render() {
    ...
    filteredData = CountriesList.filter(country => country.isVisited === true)
    console.log(CountriesList)
    return (
      <div className="countries-main-container">
        <h1 className="countries-main-heading">Countries</h1>
        <ul className="visit-country-list">
          {CountriesList.map(eachCountry => (
            <VisitCountries
              eachCountry={eachCountry}
              onClickBtn={this.onClickVisitBtn}
              key={eachCountry.id}
            />
          ))}
        </ul>
        <h1 className="countries-main-heading">Visited Countries</h1>
        {filteredData.length > 0 ? (
          <ul className="visited-country-list">
            {filteredData.map(eachCountry => (
              <VisitedCountries
                eachCountry={eachCountry}
                removeCountryBtn={this.onClickRemoveBtn}
                key={eachCountry.id}
              />
            ))}
          </ul>
